...
public class App {
    ...
    public static void main(String[] args) {
        ...
        // 5. Create Policies via createThirdPartyPolicy and createComprehensivePolicy
        System.out.println("\n=== 5. Creating Policies via Company Factory Methods ===");
        testCreateThirdParty(company, 1003, "Charlie Brown", 301, car4, 0, expDate4, "Includes towing");
        testCreateComprehensive(company, 1003, "Charlie Brown", 302, car5, 1, expDate1, 35, 2);
        // failure
        testCreateThirdParty(company, 9999, "Ghost User", 303, car4, 0, expDate4, "Invalid user"); // Invalid userID
        // failure
        testCreateComprehensive(company, 1003, "Charlie Brown", 301, car5, 1, expDate1, 35, 2); // Duplicate policyID
        ...
        InsurancePolicy foundPolicy = company.findPolicy(searchUserID, searchPolicyID);
        if (foundPolicy != null) {
        ...
        System.out.printf("Total Premium for User 1001: $%.2f%n", company.calcTotalPayments(1001));

        // 11. Print total premium payments for all users in the company
        System.out.println("\n=== 11. Total Premium for All Users ===");
        System.out.printf("Grand Total Company Premiums: $%.2f%n", company.calcTotalPayments());
        ...
        ArrayList<InsurancePolicy> userExpiredList = company.filterByExpiryDate(1001, cutoffDate1001);
        ...
        // 15. Prompt customer for date and filter all company policies by expiry date
        System.out.println("\n=== 15. Filter All Expired Policies by Custom Date ===");
        MyDate customDate = promptForDate(inputReader);
        ArrayList<InsurancePolicy> globalExpiredList = company.filterByExpiryDate(customDate);
        if (globalExpiredList.isEmpty()) {
            System.out.println("No policies expired before " + customDate.getYear() + "/" + customDate.getMonth() + "/"
                    + customDate.getDay());
        } else {
    ...
    private static void testAdminLogin(InsuranceCompany company, String user, String pass) {
    ...
    private static void testAddUser(InsuranceCompany company, User user) {
        if (company.addUser(user)) {
            System.out.println("User added successfully: " + user.getName() + " (ID: " + user.getUserID() + ")");
        } else {
            System.out.println("Failed to add user: " + user.getName() + " (Duplicate ID: " + user.getUserID() + ")");
        }
    }

    private static void testAddUserByDetails(InsuranceCompany company, String name, int id, Address addr) {
        if (company.addUser(name, id, addr)) {
            System.out.println("User created & added successfully: " + name + " (ID: " + id + ")");
        } else {
            System.out.println("Failed to create user: " + name + " (Duplicate ID: " + id + ")");
        }
    }

    private static void testAddPolicy(InsuranceCompany company, int userID, InsurancePolicy policy) {
        if (company.addPolicy(userID, policy)) {
            System.out.println("Policy ID " + policy.getID() + " added to User ID " + userID + " successfully.");
        } else {
            System.out.println("Failed to add Policy ID " + policy.getID() + " to User ID " + userID
                    + " (Invalid User or Duplicate Policy).");
        }
    }

    private static void testCreateThirdParty(InsuranceCompany company, int userID, String name, int id, Car car,
            int claims, MyDate exp, String comm) {
        if (company.createThirdPartyPolicy(userID, name, id, car, claims, exp, comm)) {
            System.out.println("ThirdParty Policy " + id + " created successfully for User " + userID);
        } else {
            System.out.println("Failed to create ThirdParty Policy " + id + " for User " + userID
                    + " (Invalid User or Duplicate Policy).");
        }
    }

    private static void testCreateComprehensive(InsuranceCompany company, int userID, String name, int id, Car car,
            int claims, MyDate exp, int age, int lvl) {
        if (company.createComprehensivePolicy(userID, name, id, car, claims, exp, age, lvl)) {
            System.out.println("Comprehensive Policy " + id + " created successfully for User " + userID);
        } else {
            System.out.println("Failed to create Comprehensive Policy " + id + " for User " + userID
                    + " (Invalid User or Duplicate Policy).");
        }
    }
}
